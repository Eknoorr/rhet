...
class PipelineConnector:
    ...
    def process_learner_audio(
        ...
        Returns a single dictionary payload formatted for the LLM system prompt.
        """
        ...
        # 2. NLP Language Analysis and Translation Gloss
        nlp_res = self.nlp_service.analyze_and_translate(
            text=raw_text,
            target_gloss_language=target_gloss_language
        )

        # 3. Structured Payload for LLM Tutor / Dialog Agent
        llm_ready_signal = {
            "transcript": raw_text,
            "reference_text": reference_text,
            "pronunciation_scores": pron_scores,
