...
with st.sidebar:
    ...
    if not st.session_state.chat_history:
        ...
        for item in st.session_state.chat_history[:8]:
            ...
            flag = get_language_flag(item.get("target_language", "Spanish"))
            if st.button(
                f"{title}",
...
_page_label = "Dashboard" if st.session_state.page == "home" else "Conversation"
st.markdown(f"""
<div class="topbar">
...
if st.session_state.page == "home":
    ...
    if st.session_state.chat_history:
        ...
        recents_html = '<div class="recents-grid">'
        for item in recent:
            rflag = get_language_flag(item.get("target_language", "Spanish"))
            rtitle = item.get("title", "Conversation")[:38]
            rlang = item.get("target_language", "")
            rlvl = item.get("level", "")
            recents_html += f"""
            <div class="recent-item">
              <div class="recent-lang">{rlang[:2].upper()}</div>
              <div class="recent-info">
                <div class="recent-title-text">{rtitle}</div>
                <div class="recent-meta-text">{rlang} {rlvl}</div>
              </div>
            </div>"""
        recents_html += '</div>'
        st.markdown(recents_html, unsafe_allow_html=True)
    ...
    for message in st.session_state.messages:
        ...
        with st.chat_message(message["role"]):
            ...
            reply = result.get("conversational_reply", "")

            # ── Conversational reply
            if reply:
                st.markdown(
                    f'<div class="rhet-label">Rhet</div>'
                    f'<div class="rhet-response">{reply}</div>',
                    unsafe_allow_html=True
                )
            ...
            # ── Pronunciation audio
            pronunciation_audio_path = result.get("pronunciation_audio_path")
            if pronunciation_audio_path and os.path.exists(pronunciation_audio_path):
                st.audio(pronunciation_audio_path, format="audio/wav")

            # ── Translation
            translation = result.get("translation", "")
            if translation:
                st.markdown(
                    f'<div class="translation-text">{translation}</div>',
                    unsafe_allow_html=True
                )
            ...
            if target_audio_path and os.path.exists(target_audio_path):
                st.audio(target_audio_path, format="audio/wav")
    ...
    if submission is not None:
        ...
        # Switch to chat page on any input
        st.session_state.page = "chat"

        if text_input and audio_input is not None:
            st.warning("Please use either text or voice for one turn.")
            st.stop()
        ...
        # ── TEXT MODE ──────────────────────────────────────────────────
            ...
            try:
                ...
                result = agent.generate_tutor_turn(learner_signal)
                ...
                target_audio = generate_pronunciation_audio(next_target, target_language)
                result["target_audio_path"] = target_audio

                target_text = result.get("conversational_reply", "")
                pronunciation_audio = generate_pronunciation_audio(target_text, target_language)
                ...
                error_result = {
                    "conversational_reply": "Sorry, I couldn't process that message.",
                    "translation": "",
        ...
        # ── VOICE MODE ─────────────────────────────────────────────────
            ...
            try:
                ...
                pronunciation_scores = getattr(response, "pronunciation_scores", None)
                feedback     = getattr(response, "feedback", "")
                native_gloss = getattr(response, "native_gloss", "")
                next_prompt  = getattr(response, "next_prompt", "")
                tutor_audio_path = getattr(response, "tutor_audio_path", None)
                ...
                assistant_result = {
                    ...
                    "translation": native_gloss,
                    "conversational_reply": next_prompt,
                    "suggested_next_target": next_prompt,
                ...
                st.session_state.next_target = next_prompt or ""
